...
export default function StickyGlassNavbar() {
  ...
  const renderNavLink = (name: string, href: string, external?: boolean) => {
    ...
    return (
      <Link
        ...
        className={`px-4 py-2 rounded-full transition-all duration-200 font-medium ${isActive
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 px-4 transition-all duration-300 ${isScrolled ? "py-2" : "py-4"
        }`}
    >
      <nav
        className={`w-full max-w-6xl mx-auto bg-stone-900/30 border border-white/10 backdrop-blur-sm rounded-full px-6 py-3 flex items-center justify-between ${isScrolled ? "shadow-md shadow-purple-400/60" : ""
          }`}
      >
        {/* Logo */}
                ...
                {NAV_ITEMS.map(({ name, href, external }) => (
                  <li key={name}>
                    <Link
                      ...
                      className={`flex items-center px-4 py-3 mb-2 rounded-xl font-medium transition-all ${pathname === href
